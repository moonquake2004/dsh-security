...
### Fixed — SR5 的成片假警报（**只在整库尺度上才暴露**）

在整库（103 个会话）上验证 SR5 时发现：**25 个会话被判为"访问凭据库"，几乎全是假警报**。
此前 SR5 只在"当前被选中的那一个会话"上验证过——这与我们在别处批评过的"只在一种输入上验证"是同一个错误，只是这次犯在自己身上。

逐条定位到的误报机制（都不是"调个参就能好"的）：

| 误报机制 | 实例 | 处置 |
|---|---|---|
| 新加的"浏览器 profile 目录"判据（`Google/Chrome\|Chromium\|…`） | **仅仅提到浏览器或路径**就命中（25 个会话全假） | **删除**（25 → 18） |
| 凭据路径 + **整条命令级**的写入判定 | `cat ~/.npmrc 2>/dev/null` 因 `2>/dev/null` 里的 `>` 被判为"写入凭据库" | 改为**按命中路径逐个判定**（动词/重定向必须紧邻该路径） |
| 裸 `.key` | 命中 jq 表达式里的 `\(.key)` | **要求路径前缀** |
| 裸 `.env` | 命中"在项目里创建/编辑 .env"这类正常开发行为 | **取消** |
| 配置载体（`.npmrc` / `settings.yaml` / `security.json`） | 从命令字符串推断"写入目标"本质上不可靠 | **整类取消** |

**结果**：整库失败会话 **25 → 2**，且这 2 个都是真阳性（真实的 `.credentials.yaml` 读取，正是 #6465 链条的前驱）。

### Changed — ⚠️ 判定面收窄

- `.npmrc` / `settings.yaml` / `security.json` 的**写入**、以及 `.env` / `.key` 的访问**不再报出**。
  这是有意为之：按 `SECURITY.md`，**高噪声假阳性是工具的漏洞**（它训练使用者忽略告警）。
  代价是这些类别的真信号也不再单独呈现——**宁缺勿滥**。
